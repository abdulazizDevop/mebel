...
async function loginAsAdmin() {
    ...
    if (!r.ok)
        throw new Error(`Admin login failed (${r.status}). Run \`python -m app.seed\` first.`);
...
async function ensureCategories(token) {
    ...
    for (let i = 0; i < defaultCategories.length; i++) {
        ...
        if (r.ok) {
            ...
            const all = (await fetch(`${API}/categories`).then((res) => res.json()));
            for (const c of all)
                map[c.name] = c.id;
        }
        else {
...
async function pushProducts(token, catMap) {
    const headers = { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` };
    let created = 0;
    let skipped = 0;
